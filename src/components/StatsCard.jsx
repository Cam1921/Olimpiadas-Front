...
export default function StatsCard({
  ...
  const borderColor =
    variant === "accent" ? "border-accent/30" : "border-cta/30";
  ...
  return (
    <div className="card p-6 relative">
      ...
      <div
        className={`absolute right-4 top-4 rounded-full border ${borderColor} p-1`}
      >
        <Icon className={`w-5 h-5 ${iconColor}`} />
      </div>

      <p className="text-slate-500">{title}</p>
      <div className={`mt-3 text-5xl font-semibold ${colorText}`}>{value}</div>
    </div>
  );
}
