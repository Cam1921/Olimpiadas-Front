...
export default function ControlFasesTable({
  areas = [],
  rolUsuario,
  onEstadoActualizado,
}) {
  ...
  const handleAction = async (areaId, accion, areaNombre) => {
    ...
    try {
      ...
      if (accion === "concluir") {
        ...
        if (!completo) {
          throw new Error(
            "No es posible concluir, faltan evaluaciones por registrar"
          );
        }
  ...
  const handleConcluir = (area) => {
    ...
    setMensajeModal(
      `¿Está seguro de concluir la fase de ${area.nombre}? Esta acción no se puede deshacer.`
    );
    setAccionPendiente({
      tipo: "concluir",
      areaId: area.id,
  ...
  const handleConfirmar = (area) => {
    setError(null);
    setMensajeModal(
      `¿Confirmar resultados de ${area.nombre}? Esta acción habilitará la emisión de certificados y publicación.`
    );
    setAccionPendiente({
      tipo: "confirmar",
      areaId: area.id,
  ...
  const handleCerrar = (area) => {
    setError(null);
    setMensajeModal(
      `¿Cerrar la fase de ${area.nombre}? Esta acción no se puede deshacer.`
    );
    setAccionPendiente({
      tipo: "cerrar",
      areaId: area.id,
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Gestión de confirmaciones y exportación de certificados por área y
          nivel
        </p>
      </div>
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            {areas.map((area) => (
              <tr key={area.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`px-2 py-0.5 text-xs rounded-full ${getColorChip(
                      area.estado
                    )}`}
                  >
                    {area.estado}
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    ...
                    {esAdmin && area.estado === "En evaluacion" && (
                      <button
