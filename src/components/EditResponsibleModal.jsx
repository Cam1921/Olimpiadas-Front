...
export default function EditResponsibleModal({
  ...
  const errClass = (field) =>
    errors[field]?.ok === false ? "border-2 border-red-500" : "";
  const getErrorMsg = (field) =>
    errors[field]?.ok === false ? errors[field].msg : null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative card w-[720px] p-8">
        ...
        {Object.keys(errors).some(
          (k) => k !== "general" && errors[k]?.ok === false
        ) &&
          !errors.general && (
            <div className="bg-red-50 border-l-4 border-red-500 p-3 mb-4 rounded">
              <p className="text-red-700 text-sm font-medium">
                ⚠️ Algunos datos ya están registrados o son inválidos. Revisa
                los campos en rojo.
              </p>
            </div>
          )}
