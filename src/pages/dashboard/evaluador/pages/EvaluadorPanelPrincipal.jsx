...
export default function EvaluadorPanelPrincipal() {
  ...
  const currentNivel = currentArea?.niveles.find(
    (n) => n.id === parseInt(selectedNivelId)
  );

  const handleNavigate = (ruta) => {
    console.log(
      `Navegando a: ${ruta} con Área: ${currentArea?.nombre} y Nivel: ${currentNivel?.nombre}`
    );
    alert(`Redirigiendo a vista de: ${ruta}`);
  };

  if (!currentArea)
    return <div className="p-6">Cargando información asignada...</div>;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <StatCard
            ...
            value={`${currentNivel?.evaluados || 0}/${
              currentNivel?.total || 0
            }`}
            color="text-blue-600"
          ...
          <StatCard
            ...
            value={
              areas.filter((a) => a.niveles.every((n) => n.pendientes === 0))
                .length
            }
            color="text-blue-600"
            icon={
          ...
          <StatCard
            ...
            value={`${Math.round(
              ((currentNivel?.evaluados || 0) / (currentNivel?.total || 1)) *
                100
            )}%`}
            color="text-blue-600"
            icon={
          ...
          <div className="p-6 border-b border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">
                Selecciona modalidad de calificación
              </h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <select
                ...
                onChange={(e) => {
                  ...
                  if (newArea?.niveles.length > 0)
                    setSelectedNivelId(newArea.niveles[0].id);
                }}
              >
                {areas.map((area) => (
