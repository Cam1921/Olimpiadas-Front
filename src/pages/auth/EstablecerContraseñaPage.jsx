...
// ⚠️ Solo front: simulamos validación de token.
// En integración real, harías:
//   await api.get("/auth/password-setup/validate", { params: { token } })
const simulateValidateToken = async (token) => {
  await new Promise((r) => setTimeout(r, 300)); // demo delay
  if (!token || token === "expired" || token === "invalid") return { ok: false };
  return { ok: true };
};

// Política HU: 12+ / 1 mayús / 1 minús / 1 número / 1 símbolo / sin espacios
...
export default function EstablecerContraseñaPage() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const res = await simulateValidateToken(token);
      if (!mounted) return;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-600">
                Por seguridad, crea una nueva contraseña antes de ingresar al sistema.
              </p>
            </div>
          </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    value={p1}
                    onChange={(e) => { setP1(e.target.value); if (!touched1) setTouched1(true); }}
                    onBlur={() => setTouched1(true)}
                    className={`w-full rounded-xl border px-3 py-2 pr-11 outline-none
                      ${touched1 && !policy.ok ? "border-red-500" : "border-slate-300 focus:ring-2 focus:ring-blue-500"}
                    `}
                    placeholder="••••••••••••"
                ...
                {!policy.ok && touched1 && (
                  <p id="policy-help" className="text-sm text-red-600 mt-1">{policy.msg}</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    value={p2}
                    onChange={(e) => { setP2(e.target.value); if (!touched2) setTouched2(true); }}
                    onBlur={() => setTouched2(true)}
                    className={`w-full rounded-xl border px-3 py-2 pr-11 outline-none
                      ${confirmErr ? "border-red-500" : "border-slate-300 focus:ring-2 focus:ring-blue-500"}
                    `}
                    placeholder="••••••••••••"
                    autoComplete="new-password"
                ...
                {confirmErr && (
                  <p id="confirm-help" className="text-sm text-red-600 mt-1">{confirmErr}</p>
                )}
              </div>

              <div className="pt-2 flex items-center justify-between">
                <Link to="/" className="text-slate-600 hover:text-slate-800 underline underline-offset-4">
                  Volver al inicio
                </Link>
                <button
                  type="submit"
      ...
      <ModalExito
        open={openSuccess}
        onClose={() => { setOpenSuccess(false); navigate("/"); }}
      />
      <ModalTokenExpirado
        open={openExpired}
        onClose={() => { setOpenExpired(false); navigate("/"); }}
