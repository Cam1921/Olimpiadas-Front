...
export default function Evaluadores({ onBackToResponsables }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        r.correo?.toLowerCase().includes(q) || r.ci?.toLowerCase().includes(q)
    );
  }, [rows, search]);
  ...
  return (
    <div className="px-4 sm:px-6 md:px-8 py-6">
      ...
      <div className="flex items-center gap-6 border-b border-slate-200 overflow-x-auto">
        ...
        <button
          className="py-3 border-b-2 border-cta text-cta font-semibold shrink-0 text-sm sm:text-base"
          aria-current="page"
        >
          Evaluadores
        </button>
      ...
      <div className="-mx-4 mt-5 sm:mx-0 overflow-x-auto">
